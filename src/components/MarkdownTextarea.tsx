...
export default function MarkdownTextarea({ name, value, onChange, rows = 12, placeholder, required }: Props) {
  ...
  const active = { backgroundColor: "#0D2B4E", color: "#FFFFFF" };
    ...
    <div className="border rounded-lg overflow-hidden" style={{ borderColor: "#D0D0D0" }}>
      ...
      {tab === "write" ? (
        ...
        <div
          ...
          style={{ color: "#0D2B4E", lineHeight: "1.75" }}
        >
          {value.trim() ? (
            <ReactMarkdown
              components={{
                h1: ({ children }) => <h1 style={{ fontSize: "1.25rem", fontWeight: 700, marginBottom: "0.5rem", color: "#0D2B4E" }}>{children}</h1>,
                h2: ({ children }) => <h2 style={{ fontSize: "1.1rem", fontWeight: 700, marginBottom: "0.4rem", color: "#0D2B4E" }}>{children}</h2>,
                h3: ({ children }) => <h3 style={{ fontSize: "1rem", fontWeight: 600, marginBottom: "0.3rem", color: "#0D2B4E" }}>{children}</h3>,
